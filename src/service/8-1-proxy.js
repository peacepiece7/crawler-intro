...
const cralwer = async () => {
  try {
    let browser = await puppeteer.launch({ headless: false, args: ["--window-size=1280,1280"] });
    ...
    console.log(proxies.filter((val) => console.log(val.type, val.type)));
    const filtered = proxies.filter((value) => value.type.split(" ")[0] === "HTTP").sort((pre, cur) => pre.latency - cur.latency);
    console.log(`--proxy-server=http://${filtered[0].ip}`);
